...
| [Fluent Bit to Alloy migration](fluent-bit-to-alloy-migration/) | Run Fluent Bit and its hand-written Alloy equivalent side by side against the same two log files and verify both produce identical labels and identical log lines in Loki. `alloy convert` has no Fluent Bit source format, so every mapping is manual. |
